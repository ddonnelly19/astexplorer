...
async function transform(transformer: any, transformCode: string, code: string) {
  // Transforms may make use of Node's __filename global. See GitHub issue #420.
  // So we define a dummy one.
  const globalWithFilename = global as typeof globalThis & {__filename?: string};
  if (!globalWithFilename.__filename) {
    globalWithFilename.__filename = 'transform.js';
  }
  if (!transformer._promise) {
    transformer._promise = new Promise(transformer.loadTransformer);
  }
  let realTransformer: any;
  try {
...
export default (store: Store) => (next: Next) => async (action: any) => {
